...
def calculate_mdd_init(history, start_bal):
    h = np.array(history)
    # Максимальное падение ниже стартовой точки
    drawdown = (start_bal - h) / start_bal
...
def run_simulation(n_sims, w_rate, silent=False):
    ...
    for i in range(int(n_sims)):
        ...
        all_runs.append({
            ...
            "mdd_init": calculate_mdd_init(balances, start_balance), # ПУНКТ 3
    ...
    if not silent: status_text.empty()
...
fig.update_layout(template="plotly_dark", height=450, margin=dict(l=20, r=20, t=20, b=20), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
st.plotly_chart(fig, use_container_width=True)

# ПУНКТ 2: Percentiles сразу после графика
