...
# --- ОБНОВЛЕННЫЙ CSS ДЛЯ ДИНАМИЧЕСКОГО ФОНА ---
st.markdown("""
    <style>
    ...
    /* СТИЛИ ДЛЯ ЦВЕТНЫХ ОКОШЕК (ПЛАШЕК) */
    .scenario-box {
        padding: 20px;
        border-radius: 15px;
        margin-bottom: 20px;
...
# --- ДЕТАЛЬНЫЙ АНАЛИЗ С ЦВЕТНЫМ ФОНОМ ---
...
def render_scenario(data, bg_class):
    # Оборачиваем метрики в div с динамическим фоном
    st.markdown(f'<div class="scenario-box {bg_class}">', unsafe_allow_html=True)
...
# Передаем нужный класс фона для каждой вкладки
